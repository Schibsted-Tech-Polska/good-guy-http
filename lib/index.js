...
var DEFAULT_SETTINGS = {
  ...
  // if the serve doesn't specify Cache-Control, default cache setup is caching for 5s
...
function goodGuyHttp(config) {
  ...
  if (config.circuitBreaking) {
    var cbConfig = { errorThreshold: config.circuitBreaking.errorThreshold || 50 };
      ...
      if (entry) {
        if (entry.expires < now()) {
          ...
          if (entry.allowServingStale && config.allowServingStale) {
            ...
            fetchInBackground(req).then(postProcessIfRequested).then(updateCache(cacheKey));
  ...
  function updateCache(cacheKey) {
    return function(entry) {
      ...
      if (entry.error) {
        ...
        cacheSettings = parseCacheSettings(entry.data) || config.defaultCaching;
      }
